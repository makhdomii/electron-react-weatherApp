...
export default class Search extends Component {
  ...
  componentWillMount = () => {
    ...
    // fetch('https://www.metaweather.com/api/location/search/?query=' + this.keywords)
    //   .then(res => res.json())
    //   .then(res => {
    //     console.log(res)
    //     this.setState({
    //       result: res, loading: false
    //     })
    //   })
    //   .catch(error => console.log(error))
    API.post(`search/?query=${this.keywords}`)
      .then(res => {
  ...
  search = () => {
    ...
    if (inputValue.length > 2) {
      ...
      // fetch('https://www.metaweather.com/api/location/search/?query=' + inputValue)
      //   .then(res => res.json())
      //   .then(res => {
      //     console.log(res)
      //     this.setState({
      //       result: res, loadApi: false
      //     })
      //   })
      //   .catch(error => console.log(error))
      API.post(`search/?query=${inputValue}`)
        .then(res => {
          this.setState({ result: res, loadApi: false });
        })
        .catch(error => console.log(error));
    }
  };
